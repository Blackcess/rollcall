...
function IndividualSubject(){
    ...
    const getSpecificSubject = async ()=>{
        try {
            const response= await axios.get(`${API_BASE_URL }/assets/results/semester/specific/subject?id=${parseInt(course_id)}&table=${indexToSemesterSubjectsTable(4)}`,{
...
const SubjectImageDisplay= styled.div`
height:200px;
width:100%;
position:relative;
border-radius:10px;
